...
function preTrainingChecks() {
  ...
  function drawNetwork() {
    ...
    for (let i = 0; i < layerVisuals.length; i++) {
        ...
        let currentX = networkAreaStartX + xSpacing * (i + 2); // Adjust for 0-based index and add 1 for input layer offset
        ...
        for (let j = 0; j < prevLayerNodes; j++) {
            for (let k = 0; k < currentLayer.nodes; k++) {
                ...
                let opacity = p.map(Math.abs(weight), 0, 1, 0, 255);
                let thickness = p.map(Math.abs(weight), 0, 1, 2, 10);
    ...
    for (let i = 0; i < layerVisuals.length; i++) {
        ...
        let x = networkAreaStartX + xSpacing * (i + 2); // Adjust for 0-based index and add 1 for input layer offset
